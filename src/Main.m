...
sampRte_mgntmtr = 10; %[Hz] *Range: sampRte_mgntmtr > 0*
...
sampRte_gyro = 10; %[Hz] *Range: sampRte_gyro > 0*
...
initBdyAnglrRteX_spcrft = 10; %[deg/s] *Range: initBdyAnglrRteX_spcrft ≥ 0.0*
initBdyAnglrRteY_spcrft = 10; %[deg/s] *Range: initBdyAnglrRteY_spcrft ≥ 0.0*
initBdyAnglrRteZ_spcrft = 10; %[deg/s] *Range: initBdyAnglrRteZ_spcrft ≥ 0.0*
...
simSpdMltplr = 20; %[unitless] *Range: simSpdMltplr > 0.0*
    ...
    case 'Real-Time'
        while (ishghandle(simUI) && t < tf)
            ...
            if (t - tPrev1 >= 1 / frmeRte_simUI)
                ...
                dqtrnion_dt = [0, -bdyAnglrRte_spcrft(1), -bdyAnglrRte_spcrft(2), -bdyAnglrRte_spcrft(3); bdyAnglrRte_spcrft(1), 0, bdyAnglrRte_spcrft(3), -bdyAnglrRte_spcrft(2); bdyAnglrRte_spcrft(2), -bdyAnglrRte_spcrft(3), 0, bdyAnglrRte_spcrft(1); bdyAnglrRte_spcrft(3), bdyAnglrRte_spcrft(2), -bdyAnglrRte_spcrft(1), 0] * qtrnion_spcrft * 0.5;
                k1 = dbdyAnglrRte_dt;
                k2 = inrtaTnsr_spcrft \ (appliedTrqe_mgntrqrs - cross(bdyAnglrRte_spcrft + k1 * (simSpdMltplr / frmeRte_simUI) / 2, inrtaTnsr_spcrft * (bdyAnglrRte_spcrft + k1 * (simSpdMltplr / frmeRte_simUI) / 2)));
                k3 = inrtaTnsr_spcrft \ (appliedTrqe_mgntrqrs - cross(bdyAnglrRte_spcrft + k2 * (simSpdMltplr / frmeRte_simUI) / 2, inrtaTnsr_spcrft * (bdyAnglrRte_spcrft + k2 * (simSpdMltplr / frmeRte_simUI) / 2)));
                k4 = inrtaTnsr_spcrft \ (appliedTrqe_mgntrqrs - cross(bdyAnglrRte_spcrft + k3 * (simSpdMltplr / frmeRte_simUI), inrtaTnsr_spcrft * (bdyAnglrRte_spcrft + k3 * (simSpdMltplr / frmeRte_simUI))));
                bdyAnglrRte_spcrft = bdyAnglrRte_spcrft + (1 / 6) * (k1 + 2 * k2 + 2 * k3 + k4) * (simSpdMltplr / frmeRte_simUI);
            ...
            if (t - tPrev3 >= 1 / sampRte_gyro)
                ...
                hold(axes_gyroPlt, 'off');
                hold(axes_mgntrqrPlt, 'on');
                crrnt_mgntrqrs = (-bDotCntrllrGain_mgntrqrs ./ (numCoils_mgntrqrs .* sctnArea_mgntrqrs)) .* cross(dsirdBdyAnglrRte_spcrft - msurdBdyAnglrRte, msurdBdyMgntcFld);
                if (abs(crrnt_mgntrqrs(1)) > maxCrrnt_xMgntrqr)
                    crrnt_mgntrqrs = crrnt_mgntrqrs * maxCrrnt_xMgntrqr / abs(crrnt_mgntrqrs(1));
                end
                if (abs(crrnt_mgntrqrs(2)) > maxCrrnt_yMgntrqr)
